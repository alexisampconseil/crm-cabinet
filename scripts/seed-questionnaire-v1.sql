-- =============================================================================
-- SEED : Questionnaire patrimonial v1.0 — AMP CONSEIL
-- =============================================================================
-- Périmètre : référentiel patrimonial uniquement.
-- Exclu intentionnellement : profil de risque, connaissances financières,
--   expérience financière, ESG, LCB-FT.
--
-- Convention prefill_path :
--   - Chemin absolu depuis snapshot_prefill pour les questions non-répétables.
--     Ex : "identite.nom", "fiscalite.tranche_ir"
--   - Chemin relatif "item.*" pour les questions d'un bloc répétable.
--     Ex : "item.libelle", "item.montant"
--   Le tableau source des instances répétables est indiqué par repete_source
--   au niveau du bloc (ex : "patrimoine_financier", "foyer.enfants").
--
-- Convention question_code :
--   Les question_code doivent rester globalement uniques par convention,
--   sauf réutilisation volontaire avec portee différente (ex : "id_nom"
--   avec portee="client" et portee="conjoint" dans le même bloc identite).
--   Le préfixe de bloc (id_, foy_, pro_, af_, …) est le garde-fou principal
--   car la contrainte unique de questionnaire_reponses ne comprend pas "bloc".
--
-- Convention conditions :
--   - champ = "perimetre" : valeur portée par la session (client_seul|foyer)
--   - champ = code d'une autre question du même bloc (portee client)
--   - Toutes les conditions d'une même question sont évaluées en AND.
--
-- Prérequis :
--   Aucune autre version avec actif = TRUE ne doit exister.
--   Si c'est le cas, exécuter d'abord :
--     UPDATE questionnaire_versions SET actif = FALSE WHERE actif = TRUE;
--
-- Idempotence :
--   ON CONFLICT (id) DO NOTHING — sans danger si rejoué.
-- =============================================================================
...
VALUES (
  'c1a55000-0000-4000-8000-000000000001'::uuid,
  '1.0',
  'Bilan patrimonial initial v1',
  'Première version du questionnaire patrimonial AMP CONSEIL. Couvre : identité civile, situation familiale, situation professionnelle, revenus, charges, patrimoine financier, patrimoine immobilier, crédits, fiscalité, prévoyance et objectifs patrimoniaux. Hors périmètre : profil de risque, connaissances financières, ESG, LCB-FT.',
  '1.0',
  $json$
{
  "blocs": [

    {
      ...
      "questions": [
        ...
        },
        {
          ...
          "conditions": [
            { "champ": "perimetre", "operateur": "=", "valeur": "foyer" }
          ]
        },
        {
          "code": "id_prenom",
      ...
      "questions": [
        {
          ...
          "options": ["celibataire", "marie", "pacse", "concubinage", "divorce", "veuf"],
          "conditions": []
        },
        {
          ...
          "conditions": [
            { "champ": "foy_situation", "operateur": "=", "valeur": "marie" }
          ]
        },
        {
        ...
        },
        {
          ...
          "options": ["commun", "client", "conjoint", "adoption"],
          "conditions": []
        }
      ]
    },
      ...
      "questions": [
        {
          ...
          "options": ["AV", "PER", "SCPI", "Capitalisation", "PEA", "CTO", "Livret", "Autre"],
          "conditions": []
        },
        {
          "code": "af_libelle",
        ...
        },
        {
          ...
          "options": ["client", "conjoint", "commun"],
          "conditions": []
        },
        {
      ...
      "questions": [
        {
          ...
          "repete": true,
          "prefill_path": "item.nature",
          "options": ["RP", "RS", "Locatif", "SCI", "Autre"],
          "conditions": []
        },
        {
          "code": "immo_valeur",
        ...
        },
        {
          ...
          "options": ["client", "conjoint", "commun", "SCI"],
          "conditions": []
        },
        {
      ...
      "questions": [
        {
          ...
          "repete": true,
          "prefill_path": "item.nature",
          "options": ["immobilier", "consommation", "professionnel", "autre"],
          "conditions": []
        },
        {
          "code": "passif_banque",
...

    {
      ...
      "questions": [
        ...
        },
        {
          ...
          "repete": true,
          "prefill_path": "item.nature",
          "options": ["deces", "incapacite", "invalidite", "dependance", "sante", "autre"],
          "conditions": []
        },
        {
...

    {
      ...
      "questions": [
        ...
        },
        {
          ...
          "options": ["court_terme", "moyen_terme", "long_terme", "retraite"],
          "conditions": []
        },
        {
...
-- =============================================================================
-- VÉRIFICATION — à exécuter après le seed
-- =============================================================================
-- SELECT id, version, libelle, statut, actif, date_publication
-- FROM questionnaire_versions
-- WHERE id = 'c1a55000-0000-4000-8000-000000000001';
--
-- SELECT jsonb_array_length(structure->'blocs') AS nb_blocs
-- FROM questionnaire_versions
-- WHERE id = 'c1a55000-0000-4000-8000-000000000001';
--
-- SELECT
--   bloc->>'code'    AS code,
--   bloc->>'libelle' AS libelle,
--   bloc->>'ordre'   AS ordre,
--   bloc->>'repete'  AS repete,
--   jsonb_array_length(bloc->'questions') AS nb_questions
-- FROM questionnaire_versions,
--      jsonb_array_elements(structure->'blocs') AS bloc
-- WHERE id = 'c1a55000-0000-4000-8000-000000000001'
-- ORDER BY (bloc->>'ordre')::int;
-- =============================================================================
