-- =============================================================================
-- SEED : Questionnaire patrimonial v1.3 — AMP CONSEIL
-- =============================================================================
-- Version dérivée de v1.2 (c1a55000-0000-4000-8000-000000000003).
-- Changements :
--   - Suppression d'une instance répétable (snapshot ou ajoutée en session) :
--     génère un écart type_ecart='suppression' (symétrique à 'ajout'),
--     validation conseiller obligatoire avant suppression réelle au référentiel.
--     Question "systeme" par bloc concerné (af/immo/passif/rev/chg/obj_supprime),
--     jamais affichée — colonne_cible=COLONNE_SUPPRESSION (mapping.ts).
--   - Ville de naissance : libellé clarifié, repositionnée sous date de naissance.
--   - Régime du PACS : option "Autre" retirée, libellé séparation de biens clarifié.
--   - Revenus : "Employeur" retiré de la catégorie Salaires (déjà en situation pro).
--   - Actifs financiers : date de souscription / mode de détention (+ démembrement)
--     conditionnés à la nature du produit — masqués pour un Livret.
--   - Immobilier : revenus locatifs + nouveau régime fiscal (micro-foncier/réel)
--     conditionnés à une nature locative (Locatif/SCI) — masqués pour RP/RS.
--   - Passifs : libellé "Capital restant dû" du montant initial renommé
--     "Capital emprunté" (désambiguïsation avec passif_capital_restant_du).
--   - Fiscalité : option barème progressif passe de booléen à 3 choix
--     (Oui/Non/Je ne sais pas) — colonne DB reste BOOLEAN, "Je ne sais pas"
--     coercé en NULL, aucune migration requise.
--
-- Aucune migration requise pour cette version (suppression : sentinel applicatif
-- uniquement ; nouveaux champs : JSONB detail déjà existant).
--
-- Idempotence :
--   1. Désactive la version active courante (v1.2).
--   2. Insère la nouvelle version 1.3, active.
...
VALUES (
  'c1a55000-0000-4000-8000-000000000004'::uuid,
  '1.3',
  'Bilan patrimonial initial v1.3',
  'Version 1.3 — suppression d\'instance comme écart symétrique à l\'ajout, questionnaire conditionnel affiné (placements financiers, immobilier locatif), corrections de libellés (ville de naissance, capital emprunté), option \'je ne sais pas\' sur le barème progressif, retrait du champ employeur pour les salaires.',
  '1.0',
  $json$
{
  "blocs": [
